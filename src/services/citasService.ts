...
import type {
  Cita,
  CitaFilters,
  PaginatedCitasResponse,
  CreateCitaData,
  UpdateCitaData,
  CalendarioCita,
} from "../types/citas";

const buildCitaParams = (filters?: CitaFilters) => {
  ...
  if (filters?.localId) params.append("localId", String(filters.localId));
...
export const citaService = {
  ...
  getAll: async (filters?: CitaFilters): Promise<PaginatedCitasResponse> => {
    const params = buildCitaParams(filters);
  ...
  getEstados: async (): Promise<Array<{ id: number; nombre: string }>> => {
